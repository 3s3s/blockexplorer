...
exports.GetTransactionsByAddress2 = function(query, res)
{
    ...
    g_utils.WaitBlockSync(()=>{ 
        g_rpc.getblockcount("", function(result) {
            ...
            g_constants.dbTables['Address'].selectAll("*", strQueryAddr, "ORDER BY height ASC LIMIT 10000", function(error, rows) {
                try
                {
                    if (error || !rows)
                    {
    ...
    g_utils.WaitBlockSync(()=>{     
        try
        {
            ...
            g_rpc.getblockcount("", function(result) {
                ...
                g_constants.dbTables['Address'].selectAll("*", "(" + strQueryAddr + ")" + " AND (txout='0' AND value<>'0')", " ORDER BY value ASC LIMIT 10000", function(error, rows) {
                    try
                    {
                        if (error || !rows)
                        {
